...
* [Queries](#queries)
  ...
  * [Things an ORM would do for you](#things-an-orm-would-do-for-you)
* [Sessions](#sessions)
...
- [PostgreSQL](https://www.postgresql.org/): A powerful, open source object-relational database system with over 35 years of active development. This provides the primary data storage, accessed via [pgx](https://github.com/jackc/pgx).
- [SQLite](https://sqlite.org/): A small, fast, self-contained, high-reliability, full-featured, SQL database engine. This backs the persistent, background [task queues](#tasks) only.

Application data lives in Postgres. The task queue is kept in a separate SQLite database because [Backlite](https://github.com/mikestefanello/backlite), the queue library, is SQLite-only -- see [tasks](#tasks). For [caching](#cache), a simple in-memory solution is provided. If you need to use something like Redis, swapping that in can be done quickly and easily.

A `docker-compose.yml` is included to run Postgres locally; `make db-up` starts it and creates both the application and test databases.
...
Several optional tools are available to make development easier for you. This includes the [sqlc](#queries) code-generator, for generating type-safe query code, the [golang-migrate](#migrations) CLI, for creating and running migrations, [Air](https://github.com/air-verse/air) CLI, to provide [live reloading](#live-reloading), and [Tailwind CSS](https://tailwindcss.com/docs/installation/tailwind-cli) CLI, to generate CSS. 
...
Run the same command again for any further admin accounts you need. Note that the password is only printed once, at creation time, since only a `bcrypt` hash of it is stored.
...
The database used for application data is [PostgreSQL](https://www.postgresql.org/), accessed through [pgx](https://github.com/jackc/pgx). A `*pgxpool.Pool` is included in the `Container` as `Database` if direct access is required, alongside `Queries`, the generated [query](#queries) interface you will normally use.

A second, separate [SQLite](https://sqlite.org/) database backs the [task queue](#tasks) and is exposed on the `Container` as `TasksDatabase`. This split exists because [Backlite](https://github.com/mikestefanello/backlite) is SQLite-only; it is not a general-purpose second datastore, and application data should not be put there.
...
```
pkg/postgres/migrations/
├── 000001_init.up.sql
└── 000001_init.down.sql
```

The CLI is still useful for authoring migrations and for operating on a database directly:

```
make migrate-new name=add_posts   # create the next pair of migration files
make migrate-up                   # apply all pending migrations
make migrate-down                 # roll back the most recent migration
make migrate-force version=1      # clear a dirty migration state
```

Those targets default to the local development database. Override `DATABASE_URL` to point them elsewhere:
...
Since many tests can require a database, this application supports a separate database specifically for tests. Within the `config`, the test database can be specified at `Config.Database.TestConnection`, which is the database connection string that will be used.

When a `Container` is created, if the [environment](#environments) is set to `config.EnvTest`, it will connect to the test database instead and run migrations so your tests start with a clean, ready-to-go database.

Because each test package is compiled into its own binary and `go test` runs packages in parallel, every container isolates itself inside a randomly-named Postgres _schema_ rather than sharing one. The `$RAND` placeholder in the test connection string is replaced with a random value at startup:

```yaml
testConnection: "postgres://pagoda:pagoda@localhost:5432/pagoda_test?sslmode=disable&search_path=test_$RAND"
```

The schema is created before migrations run and dropped again when the container shuts down, so parallel packages cannot clobber each other and no schemas are left behind. Without this, packages would drop the schema out from under one another and tests would fail intermittently.

## Queries

Rather than an ORM, data access is handled by [sqlc](https://sqlc.dev/), which generates fully type-safe Go code from plain SQL. You write the query, sqlc gives you a typed method and typed structs -- there is no query builder to learn and no runtime reflection. If you are not familiar with sqlc, their [documentation](https://docs.sqlc.dev/) is short and worth reading in full.
...
A `*pgdb.Queries` is included in the `Container` as `Queries` to provide easy access throughout the application. `emit_interface` is enabled, so a `pgdb.Querier` interface is also generated, which is useful if you want to mock data access in tests.

### Tables

The two included tables are:
- `users`
- `password_tokens`
...
1. Create the migration files:
   ...
   ```
   make migrate-new name=add_posts
   ```

2. Write the `up` and `down` SQL in `pkg/postgres/migrations`.

3. Add or update the queries in `pkg/postgres/queries`. Each query is annotated with a name and a
   result cardinality, for example:
...
That query generates a typed params struct and method, called like so:

```go
token, err := c.Queries.GetPasswordToken(ctx.Request().Context(), pgdb.GetPasswordTokenParams{
    ID:           tokenID,
    UserID:       userID,
    CreatedAfter: expiration,
})
```
...
Prior to logging a user in, the method `CheckPassword()` can be used to determine if a user's password matches the hash stored in the database on the `User` record.

Routes are provided for the user to login and logout at `user/login` and `user/logout`.

### Forgot password

Users can reset their password in a secure manner by issuing a new password token via the method `GeneratePasswordResetToken()`. This creates a new row in the `password_tokens` table belonging to the user. The actual token itself, however, is not stored in the database for security purposes. It is only returned via the method so it can be used to build the reset URL for the email. Rather, a hash of the token is stored, using `bcrypt` the same package used to hash user passwords. The reason for doing this is the same as passwords. You do not want to store a plain-text value in the database that can be used to access an account.
...
The actual registration of a user is not handled within the `AuthClient` but rather just by inserting a row into the `users` table via `Queries.CreateUser()`. When creating a user, you **must** use `HashPassword()` to create a hash of the user's password, which is what will be stored in the database -- this is not done for you. See [things an ORM would do for you](#things-an-orm-would-do-for-you).
...
An `admin` boolean column exists on the `users` table to indicate if the user has admin access. If your app requires a more robust authorization system, such as roles and permissions, you could easily replace this field and adjust all usage of it accordingly. If a user has this field checked, they will be able to access the [admin panel](#admin-panel). [Middleware](#middleware) is provided to easily restrict access to routes based on admin status.

### Authenticated user

The `AuthClient` has two methods available to get either the `*pgdb.User` or the ID of the user currently logged in for a given request. Those methods are `GetAuthenticatedUser()` and `GetAuthenticatedUserID()`.

#### Middleware

Registered for all routes is middleware that will load the currently logged in user and store it within the request context. The middleware is located at `middleware.LoadAuthenticatedUser()` and, if authenticated, the `*pgdb.User` is stored within the context using the key `context.AuthenticatedUserKey`.
...
Most web applications require the user to verify their email address (or other form of contact information). The `users` table has a `verified` column to indicate if they have verified themself. When a user successfully registers, an email is sent to them containing a link with a token that will verify their account when visited. This route is currently accessible at `/email/verify/:token` and handled by `pkg/handlers/auth.go`.
...
The _admin panel_ currently includes a section to monitor all [background tasks and queues](#tasks), provided by [Backlite](https://github.com/mikestefanello/backlite)'s UI and mounted at `/admin/tasks`.

There are no separate templates or interfaces for the admin section (see [screenshots](#screenshots)).

Users with admin [access](#access) will see additional links on the default sidebar at the bottom. As with all default UI components, you can easily move these pages and links to a dedicated section, layout, etc.

> **Note:** upstream Pagoda also ships a completely dynamic UI to manage every entity defined by _Ent_, generated via Ent's extension API. Because this project uses [sqlc](#queries) rather than an ORM, there is no schema graph to reflect over at runtime and that generic entity CRUD has been removed. If you want admin screens for your own tables, write them as ordinary [handlers](#handlers) and [pages](#pages). The task monitoring above is unaffected.
...
For this example, we'll create a new handler which includes a GET and POST route and queries the database. Start by creating a file at `pkg/handlers/example.go`.
...
- `AuthUser`: The logged-in user (`*pgdb.User`), if one
...
[Backlite](https://github.com/mikestefanello/backlite) is the library chosen to handle queueing tasks and processing them asynchronously, using [SQLite](https://sqlite.org/) as a persistent store so that tasks are never lost, can be retried until successful, and their concurrent execution can be managed.

Backlite is SQLite-only, so the queue keeps its own database, separate from the Postgres database holding application data. It is exposed on the `Container` as `TasksDatabase` and its location is configured at `Config.Database.TasksConnection`. If you would rather run everything on a single datastore, you would need to replace Backlite with a Postgres-backed queue.
